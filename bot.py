...
admin_id = list(map(int,str(os.getenv('admin_id')).split(' ')))
#FTP variables
host = os.getenv('host')
...
def get_past_link():
    try:
        arc = download_file(f"htdocs/resumenes_robot/{past}",'r')
        number = arc.readline()
        if number[-1] == '\n':
            number = number[:-1]
        link = arc.readline()
...
def save_link(curr_num,link):
    try:
        arc = download_file(f"htdocs/resumenes_robot/{past}",'w')
...
def get_unproc_post():
    arc = download_file(f"htdocs/resumenes_robot/{unprocessed}",open_type='r')
    ans = ''
    res = []
    cnt = 1
    for line in arc:
...
#if post is from admins' channel
...
def validate_command(update):
    ...
    return update.message['chat']['id'] in admin_id
...
def backup_resume(update,context):
    ...
    v = download_file(f'htdocs/resumenes_robot/{resume}','rb',encoding=False)
    if len(v.read()) > 0:
    ...
    v = download_file(f'htdocs/resumenes_robot/{unprocessed}','rb',encoding=False)
    if len(v.read()) > 0:
        context.bot.send_document(
            chat_id = CHAT_ID,
